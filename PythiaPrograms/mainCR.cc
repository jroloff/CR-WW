...
int main() {
  ...
  Hist recrateQCD("reconnection rate QCD", 40, 0., 400.);
  // Loop over reconnection model and CM energy.
  for (int iRec = 0; iRec < 3; ++iRec) {
    Hist& recrate = (iRec == 0) ? recrateQCD :(iRec == 1) ? recrateSK1 : recrateSK2;
    for (int iEcm = 14; iEcm < 40; ++iEcm) {
      ...
      if (iRec == 0){
	      pythia.settings.mode("ColourReconnection:mode", iRec+1);
      }
      else {
  ...
  cout << recrateSK1 << recrateSK2 << recrateQCD;
  ...
  hpl.add( recrateQCD, "h,black", "QCD");
  hpl.plot();
