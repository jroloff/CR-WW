...
int main() {
  ...
  // Histograms.
  Hist recrateSK1("reconnection rate SK1", 10, 10., 110.);

  // Loop over reconnection model and CM energy.
  for (int iRec = 1; iRec < 11; ++iRec) {
      Hist& recrate = recrateSK1;
      ...
      pythia.settings.mode("ColourReconnection:mode", 3);
      pythia.settings.parm("ColourReconnection:kI", iRec*10);
      ...
      for (int iEvent = 0; iEvent < nEvent; ++iEvent) {
        ...
        for (int i = 6; i < event.size(); ++i)
          // 76 : partons with momentum shuffled or a new colour to decouple junction-antijunction structures
          if (event[i].statusAbs() == 79) hasRec = true;
        if (hasRec) ++nRecon;

      // End of loops. Fill reconnection rate.
      }
      recrate.fill( iRec*10.+5., double(nRecon) / double(nEvent));
   
  }

  // Print and plot histograms.
  cout << recrateSK1;
  HistPlot hpl("plot303");
  hpl.frame( "fig303", "Reconnection rate as a function of kI for "
    "e$^+$e$^-$ $\\to$ W$^+$W$^-$" "$E_{\\mathrm{CM}}=$240 GeV", "kI", "Probability");
